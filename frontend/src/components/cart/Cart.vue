...
import UOMSelector from '@/components/items/UOMSelector.vue'
...
const availableNumPadModes = computed(() => {
  ...
  if (settingsStore.allowRateChange) {
    modes.push('rate')
  }
...
const selectedCartItem = computed(() => {
  if (cartStore.selectedItemIndex === null) return null
  return cartStore.items[cartStore.selectedItemIndex] || null
})
...
function onItemSelect(index: number) {
  ...
  if (item) {
    keyboardInput.value = String(item.qty)
  }
}
...
function onUomSelect(uom: string, conversionFactor: number) {
  if (cartStore.selectedItemIndex === null) return

  cartStore.updateItemUom(
    cartStore.selectedItemIndex,
    uom,
    conversionFactor
  )
}
...
function onKeyboardInputChange() {
  const value = parseFloat(keyboardInput.value) || 0
  onNumPadUpdate(value)
}
...
<template>
  <div class="flex flex-col h-full bg-white dark:bg-gray-900">
    ...
    <!-- Selected line: UOM selector -->
    <div
      v-if="selectedCartItem && !selectedCartItem.is_free_item"
      class="px-3 py-1.5 border-t border-gray-100 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-800/40"
    >
      <div class="flex items-center justify-between gap-2">
        <div class="min-w-0">
          <div class="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">
            {{ __('Sales UOM') }}
          </div>

          <div
            v-if="selectedCartItem.uom !== selectedCartItem.stock_uom"
            class="text-[10px] text-blue-600 dark:text-blue-400"
          >
            1 {{ selectedCartItem.uom }} =
            {{ selectedCartItem.conversion_factor.toFixed(2) }}
            {{ selectedCartItem.stock_uom }}
          </div>
        </div>

        <UOMSelector
          :item-code="selectedCartItem.item_code"
          :current-uom="selectedCartItem.uom"
          @select="onUomSelect"
        />
      </div>
    </div>
        ...
        >
          ...
          {{
            ...
            (cartStore.items.length > 0
              ? ` ${formatCurrency(cartStore.roundedTotal)}`
              : '')
          }}
        </button>
      </div>
